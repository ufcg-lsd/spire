...
- The `k8s_bundle` notifier plugin can now keep API service CA bundles up to dat
e
- SPIRE Server internal cache reload timing can now be tuned (experimental) (#2169)

### Changed
- Prometheus metrics that are emitted infrequently will no longer disappear afte
r emission (#2239)
- The `k8s-workload-registrar` now uses paging to support very large deployments
 of 10,000+ pods

### Fixed
- Fixed a bug that sometimes caused new agents to not receive their full set of
selectors (#2242)
